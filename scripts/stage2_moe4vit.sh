#!/usr/bin/env bash
# Stage2 — retrain classifier on frozen MoE4ViT-Base features.
#
# Usage:
#   STAGE1_CKPT=results/.../ckp_best.pth.tar ./stage2_moe4vit.sh                 # cifar10 + linear
#   STAGE1_CKPT=... CLASSIFIER=adaptive_bls ./stage2_moe4vit.sh cifar100
set -euo pipefail
...
CUDA_VISIBLE_DEVICES="${GPU}" python "${REPO_ROOT}/train_stage2.py" \
  ...
  num_experts 8 share_experts 2 top_k 2 \
